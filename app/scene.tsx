...
import {t,getLang,label} from './i18n';
...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError(t('This browser could not start the 3D viewer. Please try a browser with WebGL enabled.'));return;}
  ...
  renderer.domElement.setAttribute('aria-label',t('Interactive human anatomy. Drag to orbit, pinch or scroll to zoom, and tap a structure to inspect it.'));
  ...
  const loadChunk=async(ci:number)=>{
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error(t('Could not assemble anatomy geometry.'));geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:t('Could not load the anatomy.'));}})();
  ...
  const contextLost=(e:Event)=>{e.preventDefault();onError(t('The 3D session was paused by your device. Reload to continue.'));};renderer.domElement.addEventListener('webglcontextlost',contextLost);
