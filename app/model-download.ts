...
export async function decodeModelResponse(response:Response,expectedBytes:number,compressed:boolean):Promise<ArrayBuffer>{
 if(!response.ok)throw new Error(t('An anatomy file could not be loaded.'));
 ...
 if(buffer.byteLength!==expectedBytes)throw new Error(t('An anatomy file was incomplete. Please reload the viewer.'));
 return buffer;
}
